...
#         print("dones_agent[{}].size() {}".format(agent_num, dones_agent[agent_num].size()))
        ...
        actions_next = torch.cat([self.agents[i].actor_target(next_states_agent[i]) for i in range(num_agents)],dim=1)
        #print("actions_next.size(), {}".format(actions_next.size(), actions_next))
        
        critic_states_next = torch.cat([next_states, actions_next],dim=1)
        ...
        Q_targets_next = self.critic_target(critic_states_next,critic_actions_next)
        ...
        Q_expected = self.critic_local(critic_states,actions_agent[agent_num])
...
#         print("actions_pred_agent[{}].size() {}".format(agent_num,actions_pred_agent[agent_num].size()))      
        actor_loss = -self.critic_local(torch.cat((states, actions_pred),dim=1), actions_pred_agent[agent_num]).mean()
